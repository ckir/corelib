...
// Reads each file, asserts no forbidden napi token appears outside the
// allowlisted `cfg_attr(feature = "napi"` derive on wire structs.
```
...
## 7. The do-now payoff — pure-Rust loopback delivery test

The concrete justification for doing this now. Add a pure-Rust integration test that:
...
2. Points the provider **driver** (`alpaca_driver`) at that server's `ws://127.0.0.1:<port>` via the existing `base_url` override.
...
This exercises the full **frame → parse → CoreEvent → channel** delivery path **in pure Rust, with no Node and no `ws` library** — exactly the path the Epic-5 transport probe could not validate (`probe-harness-loopback-no-delivery`: ws:// frames did not transit Rust `tokio-tungstenite` ↔ Node's `ws`). Closing that gap is a real, bankable outcome of Phase A.
...
1. **Lint scaffold first (red):** add the boundary lint test with the engine source set enumerated; confirm it passes against today's tree (the engine files are already napi-free) — this pins the boundary before any movement.
2. **Seam:** add the `napi` feature + convert the three payload structs to `cfg_attr`. Verify the byte-identity test + full `cargo test` stay green; verify `napi build` still emits the same `index.d.ts` shape for these objects.
3. **Boundary tightening:** isolate the wire payload structs from `CoreEvent`/`RawPricing` (so the napi-free channel types and the cfg_attr wire structs are clearly separated), updating imports. Lint + tests green.
4. **Loopback test (Alpaca):** add the tokio loopback server harness + the Alpaca delivery test. Green.
5. **Loopback fast-follow:** Finnhub + Yahoo delivery tests on the same harness. Green.
6. **Docs:** update the streaming module doc comment + ROADMAP to record Phase A done and the crate lift as the remaining deferred step.

Heavy verification (the full Rust gate: `cd rust && cargo test -- --test-threads=1`, plus the 9-cell matrix where relevant) is **offloaded by dispatching `dev-offload.yml` at milestones** (after steps 2, 4, and 6), per the chosen verify mode. Note the Rust gate is single-threaded (global flight-recorder ring).
...
- **Milestone:** dispatch `dev-offload.yml` (commit + nonce, run-name resolution) for the full remote matrix at steps 2, 4, 6.
...
|---|---|---|
| `cfg_attr` changes the `index.d.ts` object shape | Med | Diff `index.d.ts` before/after step 2; the derive output is identical when the feature is on (default), so it must not change. Gate on it. |
...
- Finnhub/Yahoo are in-scope for the loopback fast-follow (step 5) but if time-boxed, Alpaca-only still satisfies the "closes the loopback gap" success criterion.
...
3. A pure-Rust tokio loopback test proves `frame → CoreEvent` delivery for **at least Alpaca**, with no Node/`ws` — closing `probe-harness-loopback-no-delivery`.
...
5. ROADMAP records Phase A complete and the crate lift as the sole remaining deferred step of roadmap (d) Phase 3.
