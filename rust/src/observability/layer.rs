...
pub fn init_flight_recorder() {
    INIT.call_once(|| {
        ...
        let filter = EnvFilter::try_from_env("CORELIB_LOG").unwrap_or_else(|_| {
            EnvFilter::new("corelib_rust=trace,corelib_rust::stream::tick=off")
        });
    ...
    std::panic::set_hook(Box::new(move |info| {
        ...
        while let Some(ev) = r.pop() {
            let _ = writeln!(
                err,
                "[flight] {} [{}] {} {} {}",
                ev.ts_ms, ev.level, ev.target, ev.message, ev.fields
            );
        }
        let _ = err.flush();
        prev(info);
...
mod tests {
    ...
    fn emitted_trace_event_lands_in_ring() {
        ...
        assert!(lines
            .iter()
            .any(|l| l.contains("flighttest") && l.contains("pump event") && l.contains("symbol")));
    }
}
